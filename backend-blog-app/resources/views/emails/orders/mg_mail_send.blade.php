...
<p>You have 48 hours to send the payment.</p>

<p>
Go to  MG , use the receiver name, Pay the amount ${{ $grandTotal }}.00
( MG will charge their fee in addition to this)
</p>
...
<p>RECEIVER FIRST NAME: {{ $moneyReceiver->first_name }}</p>
@if($moneyReceiver->middle_name)
    <p>RECEIVER MIDDLE NAME: {{ $moneyReceiver->middle_name }}</p>
@endif
<p>RECEIVER LAST NAME:  {{ $moneyReceiver->initial_name }}</p>
<p>LOCATION: {{ $moneyReceiver->country }}</p>
